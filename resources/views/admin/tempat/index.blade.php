...
<div class="space-y-8 animate-fade-in">
    ...
    @if (session('success'))
        <div class="relative overflow-hidden bg-emerald-50 border border-emerald-100 text-emerald-700 px-6 py-4 rounded-2xl text-sm font-bold flex items-center gap-3 shadow-sm">
        ...
        <form method="GET" class="relative z-10">
            <div class="grid lg:grid-cols-4 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <select name="sektor"
                        ...
                        @foreach ($sektors as $key => $label)
                            <option value="{{ $key }}" @selected(request('sektor') == $key)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="desa"
                            ...
                            @foreach ($desas as $desa)
                                <option value="{{ $desa->id }}" @selected(request('desa') == $desa->id)>{{ $desa->nama_desa }}</option>
                            @endforeach
                        </select>
                    </div>
                @endif
                ...
                <div class="flex items-end gap-3">
                    <button class="flex-1 bg-slate-900 hover:bg-slate-800 text-white rounded-2xl font-bold py-3 transition-all active:scale-95 shadow-lg shadow-slate-200">
                        Filter
                    </button>
    ...
    <div class="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
        @if ($tempats->total() > 0)
            ...
            <div class="px-8 py-6 border-b border-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/30">
                ...
                <form method="GET" class="flex items-center gap-3">
                    ...
                    <select name="per_page" onchange="this.form.submit()"
                        ...
                        @foreach([10, 25, 50, 100] as $v)
                            <option value="{{ $v }}" {{ request('per_page', 10) == $v ? 'selected' : '' }}>{{ $v }}</option>
                        @endforeach
                    </select>
                </form>
            </div>
                            ...
                            <tr class="group hover:bg-slate-50/50 transition-all">
                                ...
                                <td class="px-8 py-6">
                                    ...
                                    @if ($tempat->nama_pemilik)
                                        <div class="flex items-center gap-1.5 text-[11px] text-slate-400 mt-1 font-bold uppercase tracking-tight">
                                ...
                                <td class="px-8 py-6">
                                    <span class="inline-flex px-3 py-1.5 rounded-xl bg-blue-50 text-blue-700 text-[10px] font-black uppercase tracking-widest border border-blue-100">
                                ...
                                <td class="px-8 py-6">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('admin.tempat.destroy', $tempat->id) }}" method="POST"
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button class="p-2 rounded-xl bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white transition-all shadow-sm">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
